...
- `supabase/tests/phase-10-bulk-ops.sql` — pgTAP: the `permanent_drop_slot` /
  ...
  the Harnwell rejection, permanent-feed removal (including the cap/conflict
  skip-set re-flag), and re-drop+re-pickup — 46
...
permanent_pickup_slot(p_picking_user_id uuid, p_assigned_block_ids uuid[],
                      p_skipped_block_ids uuid[]) RETURNS jsonb
  -- 1. Harnwell precheck: if ANY touched block's house is 'harnwell' and the
  --    picker's home_house_id <> 'harnwell' -> RAISE 'harnwell_training_required'
  --    (whole request rejected; no partial write — invariant #1).
  -- 2. UPDATE ... SET user_id=:picker, status='claimed', vacancy_origin='none',
  ...
  --    WHERE block_id = ANY(p_assigned_block_ids)
  --      AND status='vacant' AND vacancy_origin='permanent_drop';  -- race-safe
  -- 3. UPDATE ... SET vacancy_origin='temporary_drop'   -- feed-removal write
  --    WHERE block_id = ANY(p_skipped_block_ids)
  --      AND status='vacant' AND vacancy_origin='permanent_drop';  -- race-safe
  -- 4. RETURNS {assigned_count:int, skipped_count:int}.  (counts reflect only
  --    rows still in the permanent-drop vacancy at submit; weeks claimed away
  --    between popup and submit are skipped by both predicates.)
```

`p_assigned_block_ids` / `p_skipped_block_ids` are
`evaluatePermanentPickup(fresh snapshot).{assignedBlockIds, skippedBlockIds}` —
the queued and skip sets after the transaction-time re-run. The
`vacant`/`permanent_drop` predicate is the SQL backstop for the concurrent-claim
race; the per-week conflict/cap logic is not re-implemented in SQL (C6a
anti-drift). A picked-up (or claimed-away) week leaves the permanent feed because
its `status`/`vacancy_origin` changes; a **cap/conflict-skipped** week does NOT
change owner, so the pickup must explicitly re-flag it
`permanent_drop -> temporary_drop` (step 3). That re-flag is the §8.4.3 / ARCH
§7.2-step-8 guarantee: after any pickup the slot leaves the permanent openings
feed (`permanent_openings_feed` queries `vacancy_origin='permanent_drop'`)
regardless of completeness, while staying vacant so `weekly_open_shifts_feed`
still surfaces it — "partial pickups are final," handled exclusively via the
weekly pathway.
...
| 14  | Pickup is race-safe + atomic, and removes the WHOLE slot from the permanent feed | `permanent_pickup_slot` claims a block only while it is still `vacant`+`permanent_drop`; `assigned_count` reflects the rows actually claimed (partial when a week was claimed away). Zero eligible weeks still succeeds (`assigned_count` 0). In the SAME transaction it re-flags the cap/conflict skip set `permanent_drop`→`temporary_drop` (`skipped_count`), so the slot leaves the permanent feed regardless of completeness while the skipped weeks stay vacant for the weekly feed. Skipped weeks are NOT permanently re-pickable. | ARCH §7.2 step 6 race-safe predicate + step 8 feed removal; BSpec §8.4.3 ("removed from the permanent openings feed regardless of whether complete or partial … partial pickups are final … handled exclusively through the weekly open-shifts pathway"). §10 atomicity. |
...
| E6. Partial-pickup feed removal — cap/conflict-skipped week re-flagged `permanent_drop`→`temporary_drop`: leaves the permanent feed regardless of completeness, surfaces in the weekly feed, not re-pickable | 7          |
