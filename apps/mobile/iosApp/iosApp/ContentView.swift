...
final class HouseObservable: ObservableObject {
    ...
    /// Supply the shown week's seats: live fetch on the backend path, the deterministic
    /// demo week otherwise. `setWeekSeats` ignores a stale fetch (the user navigated away).
    func loadWeek() async {
        let off = vm.uiState.value.weekOffset
        let anchor = vm.uiState.value.anchor
        let seats: [HouseSeat]
        if let repo, let uid = userId {
            seats = (try? await repo.fetchHouseScheduleForWeek(userId: uid, anchor: anchor))?.seats ?? []
        } else {
            seats = DemoFactory.shared.houseWeekSeats(anchor: anchor)
        }
        vm.setWeekSeats(forOffset: off, seats: seats)
    }
...
struct ShiftsRootView: View {
    ...
    private func swapSideBox(_ label: String, _ side: SwapSide?, bg: Color, accent: Color, _ c: ShiftColors) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Text(side?.dayLabel ?? (side == nil ? "Nothing back" : "")).font(ShiftFont.sans(12)).foregroundColor(c.sec)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var houseTab: some View {
        ...
        .task(id: st.weekOffset) { await houseModel.loadWeek() }
    ...
    private func houseGrid(_ grid: HouseGridWeek, _ c: ShiftColors) -> some View {
        ...
        return VStack(spacing: 0) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                HStack(alignment: .top, spacing: 0) {
                    houseTimeRail(startHour, endHour, gridHeight, c)
    ...
    private func houseTimeRail(_ startHour: Int, _ endHour: Int, _ gridHeight: CGFloat, _ c: ShiftColors) -> some View {
        ZStack(alignment: .topTrailing) {
    ...
    var body: some View {
        ...
        ShiftSheet(title: notice.title, onClose: onClose) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(notice.dayLabel).font(ShiftFont.sans(12.5, .medium)).foregroundColor(c.sec)
                    HStack(spacing: 8) {
