...
- **HEAD moved mid-session** `5bd9f0f → d40217c`: a concurrent process merged the
  `fix/pgtap-period-overlap` commits (`e9e4e58` pgtap-overlap fix + `d40217c` Quad seed re-anchor)
  into `design/ui-implementation`. Verified my `seed.sql` and `helpers.ts` edits are **purely
  additive on top of d40217c** (no `-` lines → d40217c's work preserved) before committing.
