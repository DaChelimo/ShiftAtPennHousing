...
- `packages/core/tests/phase-06/minimum-chunk.test.ts` — exclusively
  exercises the minimum chunk rule at every selection step (~11 cases).
  NOTE (2026-06-30): the floor was lowered 2 blocks → 1 block, so these
  cases now assert that a single coverable block IS absorbed by a float;
  a block reaches Allied only when no worker can cover it at all.
...
| #   | Topic                                                                                           | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Tentative counter scope                                                                         | **GLOBAL per source**, not per-block. After each selection, increment a single counter on the source UNCONDITIONALLY (regardless of the selected span's length or block positions). The floor check is `min(effectiveHeadcount[b]) over span − globalCount ≥ 2`. Implication: a 2-worker source can spare exactly one floater per pass; a 3-worker source can spare two; etc. — even when the floaters cover disjoint blocks. BSpec §6.2 worked-example fixtures that name multi-floater scenarios at a 2-worker source (e.g., the "B 19:00–21:00 + D 21:00–24:00 from Harnwell" example) MUST set the test fixture's headcount ≥ 3 to be expressible; the BSpec example does not pin the source headcount. | BSpec §6.2 worked example explicitly increments a single counter per selection regardless of block coverage ("first floater… counter = 1 → second floater… counter = 2 → third worker ineligible"). ARCH §5.2 step 3d uses "per-block" language but the worked example only holds under GLOBAL accounting (per-block would admit a 3rd disjoint-span floater). When the spec wording and worked example diverge, the test suite locks the spec's operational behavior. The unconditional-increment clarification was added in the phase-06 audit pass after a previous heuristic ("only increment for 2-block selections") was found to silently diverge from this decision.                                                                                                                       |
...
| 14  | Minimum-chunk precondition for tiebreaker (floor = 1 block as of 2026-06-30)                     | The minimum-chunk rule is a precondition checked BEFORE the §6.3 tiebreaker chain runs, not an after-the-fact filter. **The floor was lowered 2 blocks → 1 block on 2026-06-30**, so a candidate covering a single block IS now a valid candidate; only a candidate covering zero blocks of the selected span is excluded. Constant: `MIN_FLOAT_CHUNK_BLOCKS = 1` in `packages/core/src/float-lookup/index.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                    | ARCH §5.3: "precondition for being in the candidate set." Locked in `minimum-chunk.test.ts` and `tiebreaker.test.ts`. Lowered to absorb single-block gaps into floats and minimize Allied procurement (BSpec §6.2 #4).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 16  | Partial-coverage fallback — tiered selection                                                    | The fallback applies in this order against the current uncovered run: (1) FULL coverage — workers covering the whole run; (2) LEADING coverage — workers covering the run's start with a leading portion >= `MIN_FLOAT_CHUNK_BLOCKS` (1 block as of 2026-06-30) (pinned #13); (3) LARGEST CONSECUTIVE coverage anywhere within the run, >= `MIN_FLOAT_CHUNK_BLOCKS`, with a NON-TRAILING filter on the FIRST iteration at each source (a span ending at the run's last block is excluded on iter 1; allowed on subsequent iters once the source has selected at least once). Each tier passes its tied candidates through the §6.3 tiebreaker chain. If all three tiers yield no candidate, the algorithm stops at this source and moves on.                                | BSpec §6.2 #2 ("largest consecutive sequence") and §6.2 #5 ("longest leading portion") are textually presented as alternatives but operationally compose: the leading-portion criterion is preferred, with largest-consecutive-anywhere as a further fallback when no leading candidate exists. Integration Scenario 9 ("interior 1-block hole goes to Allied") and the `partial-coverage.test.ts` cross-source scenario depend on this. The non-trailing-on-first-iter filter prevents a trailing-only worker at one source from preempting a fuller-coverage worker at the next source (locked by "Quad fallback yields partial coverage; Harnwell picks up the rest"). Added in the phase-06 audit pass to formalize an existing implementation behavior that wasn't in the original spec text. |
...
Floor lowered 2 → 1 block on 2026-06-30; these cases now assert single-block absorption.

| Test name                                                                                                  | Spec                         | Known trap                                                                                                   |
| ---------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `a sole candidate whose largest consecutive coverage is 1 block IS assigned that block`                    | BSpec §6.2 #4                | Implementer still sends the 1-block gap to Allied under the old 2-block floor                                |
| `two single-block-coverage candidates are BOTH assigned their blocks`                                      | Same                         | Same                                                                                                         |
| `a candidate with non-contiguous 1-block coverage covers its leading block`                                | Same                         | "Largest consecutive" — same worker cannot be reselected for a second stranded block                        |
| `a candidate with EXACTLY 2 consecutive blocks of coverage IS assigned both blocks`                        | Same                         | Uncovered ends still go to Allied (no coverer)                                                               |
| `after worker A takes the long run, worker B covering the last block IS assigned it`                       | BSpec §6.2 #4 (iterative)    | Implementer applies the old floor on later iterations and strands the trailing single block                  |
| `iteration halts when no uncovered blocks remain`                                                          | Same                         | Algorithm loops forever or recurses incorrectly when nothing more can be assigned                            |
| `Quad covers [0..3] of a 5-block gap; a Harnwell candidate covers [4] and IS assigned it`                  | BSpec §6.2 #4 + cross-source | Cross-source single-block leftover is now absorbed, not left to Allied                                        |
| `a full-gap coverer is selected over a 1-block-only worker`                                                | ARCH §5.3 tier order         | Full coverage still outranks a 1-block partial (tier ordering, not a floor exclusion)                         |
| `when no worker covers a full run, a worker covering the leading block IS selected`                        | BSpec §6.2 #4 + #5           | Fallback now takes a 1-block leading portion                                                                  |
| `a 2-block longest leading portion IS taken in the partial-coverage fallback`                              | Same                         | — fallback still prefers the longest leading portion                                                          |
| `worker A covers blocks [0,1,2], worker B covers [4,5,6]; block [3] has no coverer and goes to Allied`     | BSpec §6.2 #6                | A block reaches Allied only when NO worker can cover it, never merely for being 1 block                       |
...
| `a worker whose leading-portion length is only 1 block IS selected by fallback`                                              | BSpec §6.2 #4 + #5                                                                    | Under the 1-block floor a single-block leading portion is now taken by the fallback                                                |
...
| `Scenario 2 — single-block coverage is absorbed by a float`                   | 1-block minimum + Allied tail                                       |
