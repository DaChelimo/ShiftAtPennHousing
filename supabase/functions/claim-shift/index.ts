...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!/^(?:\/claim-shift)?\/claim-shift$/.test(pathname)) {
  ...
  if (claimType !== 'temporary' && claimType !== 'permanent') {
  ...
  const { data: claimedAssignmentId, error: claimError } = await supabase.rpc('claim_open_shift', {
    p_assignment_id: assignmentId,
    p_user_id: user.id,
