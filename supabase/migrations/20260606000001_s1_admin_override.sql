-- Migration: S1 — Admin override (live-calendar inline assign / reassign / remove).
--
-- BSpec §4.3 (Phase-3 override), §11.1, §1.2/§1.5 (invariants), §9.3 (caps).
-- Two SECURITY DEFINER RPCs the web admin surface calls via the service client:
--   admin_assign_worker  — fill a vacant seat / reassign an occupied non-float seat,
--                          this_week (clicked span) or permanent (every future
--                          in-semester occurrence of the (house, NY-DOW, NY-time) slot).
--   admin_remove_worker  — vacate a seat (this_week → temporary_drop; permanent →
--                          permanent_drop). Writes NO block_step_status (the
--                          orchestrator re-escalates emergently from the vacancy).
--
-- Hard blocks RAISE P0001 with a snake_case message; soft advisories (cannot /
-- opted_out / soft_cap / over_target) gate on p_override_advisories — when a soft
-- advisory applies and the flag is false the function performs NO write and returns
-- {needs_confirm:true, advisories:[...]} (no RAISE).
--
-- Reuse: effective_weekly_cap (D9 projection), permanent_pickup_slot /
-- permanent_drop_slot (permanent mechanics + the SM/SW people alerts). The Harnwell
-- training trigger (shift_block_assignments_enforce_harnwell_training) is the
-- absolute DB backstop and fires even for service_role — same-house scope satisfies
-- it by construction (D8). The valid_vacancy_origin CHECK is kept satisfied on every
-- write.

-- ===========================================================================
-- Shared helper: project the weekly cap (D9) over a set of blocks for a worker.
-- Returns the worst case across every NY calendar week the blocks touch:
--   over_hard  TRUE when any week would exceed a HARD cap (absolute, D2/§9.3).
--   over_soft  TRUE when any week would exceed a SOFT cap (advisory).
-- existing = the worker's counting-status seats already in that week (excluding the
-- blocks being added, which are matched by block_id); added = the blocks landing in
-- that week. Mirrors claim_open_shift's per-week counting.
-- ===========================================================================
CREATE OR REPLACE FUNCTION admin_override_cap_assessment(
  p_user_id uuid,
  p_block_ids uuid[]
)
RETURNS TABLE (
...
AS $$
  WITH added AS (
    SELECT
      sb.block_id,
      date_trunc('week', sb.block_start_at AT TIME ZONE 'America/New_York')::date AS week_start_date,
      sb.block_start_at
  ...
  per_week AS (
    SELECT
      added.week_start_date,
      MIN(added.block_start_at) AS sample_block_start_at,
      COUNT(*)::integer AS added_blocks
    FROM added
    GROUP BY added.week_start_date
  ),
  assessed AS (
    SELECT
      per_week.week_start_date,
      per_week.added_blocks,
      (
        ...
        WHERE existing.user_id = p_user_id
          ...
          AND existing_block.block_start_at
              >= per_week.week_start_date::timestamp AT TIME ZONE 'America/New_York'
          AND existing_block.block_start_at
              < (per_week.week_start_date + 7)::timestamp AT TIME ZONE 'America/New_York'
          AND NOT (existing.block_id = ANY (COALESCE(p_block_ids, ARRAY[]::uuid[])))
      ) AS existing_blocks,
      cap.hours_cap,
      cap.cap_enforcement
    FROM per_week
    CROSS JOIN LATERAL effective_weekly_cap(per_week.week_start_date, per_week.sample_block_start_at) AS cap
  )
  SELECT
    COALESCE(bool_or(
      cap_enforcement = 'hard'
      AND ((existing_blocks + added_blocks)::numeric * 0.5) > hours_cap
    ), false) AS over_hard,
    COALESCE(bool_or(
      cap_enforcement = 'soft'
      AND ((existing_blocks + added_blocks)::numeric * 0.5) > hours_cap
    ), false) AS over_soft
  FROM assessed;
$$;

REVOKE ALL ON FUNCTION admin_override_cap_assessment(uuid, uuid[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION admin_override_cap_assessment(uuid, uuid[]) TO service_role;
...
BEGIN
  ...
  -- ---- Write -------------------------------------------------------------
  IF p_scope = 'this_week' THEN
    -- Reassign-or-fill each seat: one row per seat, prefer a vacant seat, else a
    -- reassignable occupied non-float seat. Fill the FIRST such seat per block.
    WITH chosen AS (
  ...
  ELSE
    ...
    -- people alerts), then pick up the slot for the new worker.
    SELECT sba.user_id
...
REVOKE ALL ON FUNCTION admin_assign_worker(uuid, uuid[], uuid, text, boolean, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION admin_assign_worker(uuid, uuid[], uuid, text, boolean, timestamptz) TO service_role;
...
BEGIN
  ...
  -- block_started: any clicked seat at/after its start.
  SELECT bool_or(sb.block_start_at <= p_now)
    INTO v_has_started
  FROM shift_blocks sb
  WHERE sb.block_id = ANY (p_block_ids);
...
REVOKE ALL ON FUNCTION admin_remove_worker(uuid, uuid[], uuid, text, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION admin_remove_worker(uuid, uuid[], uuid, text, timestamptz) TO service_role;

-- rollback:
-- DROP FUNCTION IF EXISTS admin_remove_worker(uuid, uuid[], uuid, text, timestamptz);
-- DROP FUNCTION IF EXISTS admin_assign_worker(uuid, uuid[], uuid, text, boolean, timestamptz);
-- DROP FUNCTION IF EXISTS admin_override_cap_assessment(uuid, uuid[]);
