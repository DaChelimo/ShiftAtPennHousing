-- Migration: admin permanent-assign fills plain-vacant (never_assigned) future occurrences
--
-- Bug: admin_assign_worker(scope='permanent') delegated its write to
-- permanent_pickup_slot, whose WHERE clause only claims seats with
-- status='vacant' AND vacancy_origin='permanent_drop' (the worker
-- permanent-pickup feed semantics, BSpec §8.4.3). When the future occurrences
-- of the clicked slot were ordinary generated opens ('never_assigned' vacant
-- seats — the normal state of an open shift that was never permanently
-- dropped), zero seats matched and the RPC returned assigned_count=0: the HM's
-- "assign permanently" silently did nothing.
--
-- Fix: the permanent write now claims ANY vacant seat (one per future target
-- occurrence) regardless of vacancy_origin. permanent_drop seats — including
-- those just vacated by the incumbent permanent_drop_slot in the REPLACE path —
-- remain a subset of 'vacant' and are still filled (preferred via ORDER BY).
-- Same-house + hard-cap guards are unchanged and run before this branch; the
-- Harnwell training invariant is covered by the existing same-house check.
--
...
BEGIN
  ...
  ELSE
    ...
    WITH chosen AS (
      ...
      ORDER BY sba.block_id,
        CASE WHEN sba.vacancy_origin = 'permanent_drop' THEN 0 ELSE 1 END,
        sba.assignment_id
    )
    UPDATE shift_block_assignments sba
    SET status = 'claimed',
