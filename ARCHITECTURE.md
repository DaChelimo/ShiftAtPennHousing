...
**Orphaned compiled profiles (migration 20260811000002).** `operating_calendar` (and the other three runtime config tables) has no FK to `operating_seasons` by design — compiled runtime config is decoupled from the authoring row that produced it. If an `operating_seasons` row is ever removed by hand (rather than through a product flow, since none exists) without also removing what it compiled, the leftover `s_<slug>_<YYYYMMDD>` rows silently outlive it and can collide with a later season's date range: `apply_compiled_season`'s own collision guard (§ above) then blocks that later season from applying, since it sees dates already claimed by a profile that matches no current season's slug. This happened for real on 2026-08-11, when an orphaned `s_summer2026_...` profile blocked Fall 2026 from publishing.

Two admin-gated RPCs (`list_orphaned_season_profiles`, `delete_orphaned_season_profile`; both take `p_calling_user_id` and check `user_is_admin`, `REVOKE ALL FROM PUBLIC` / `GRANT TO authenticated, service_role`, mirroring `apply_compiled_season`'s own grant shape) surface this on `/admin/operations` instead of requiring manual SQL. A profile is "orphaned" when its name matches `s_%` but no current `operating_seasons` row owns it (`profile_name LIKE 's_' || slug || '_%'` for any row). Deletion is deliberately narrow: it only ever removes `operating_calendar`, `staffing_patterns`, `float_routing`, `break_periods`, and `operating_profiles` — tables with no downstream FK dependents — and re-verifies orphan status at delete time (race guard). It unconditionally **refuses** if a profile still has a `scheduling_periods` row, since that table has real dependents (draft assignments, preferences, period targets, per-house publish rows) that must be resolved by hand rather than risk a silent cascade of live data. The web action (`deleteOrphanedSeasonProfile`, `apps/web/lib/actions/operatingSeasons.ts`) requires a client-side `window.confirm` naming the row counts before calling the RPC, mirroring the break-removal confirm pattern.
...
**Rotor page/action access is narrower than `isHouseAdmin` (amended 2026-08-11).** Unlike people admin, leave, and the weekly cap — which the RSM shares with the HM/BM via `isHouseAdmin` — viewing and planning the HMOD rotor is gated by a dedicated `canManageHmodRotor(user)` predicate (`apps/web/lib/auth.ts`) that admits only `hm`/`bm`/`admin`. The `/admin/rotor` page, its nav entry, and the `saveRotor` server action all use this predicate instead of `isHouseAdmin`. Do not widen it back to `isHouseAdmin` — that was the pre-2026-08-11 bug that let an RSM view/save the rotor despite never being HMOD-eligible.
...
**Which season and week the builder is pointed at (`lib/data/buildTarget.ts`, corrected 2026-08-12).** The target is resolved **season first, week second**, by one shared helper, `resolveBuildTarget`, used by both the manual builder (`getBuilderData`) and the AI generator (`getAiScheduleContext`) so the two can never disagree about what is being built. The period is the one starting **latest** among periods that have not already ended and that this house has not published (`period_house_publications`); it falls back to the latest live period, then to the latest period overall, so a fully-published house renders its last built week instead of a blank screen. `scheduling_periods.published_at` is deliberately not the filter — it flips only once **every** house has published, so it stays null on a season this house has finished. The build week is then the week of that period's earliest non-voided block **at this house**, so a house that opens partway into a season anchors on the week it opens, and a house with no window that season yields no week at all (correctly, there is nothing to build).

Both call sites previously anchored on "the week of the earliest block this house has", an all-time minimum with no relation to now. That is correct only while exactly one season of blocks is materialized. Once `apply_compiled_season` (§2.1) materializes a second one, the anchor stays pinned to the **oldest** season forever: with Summer (desk opens 05:30) and Fall (opens 08:00) both live, an admin preparing Fall got a June week on a 05:30 axis, the summer period's preferences, and a Publish button wired to the summer period. The same read also resolved "the period covering this date" from an **unordered** `SELECT` taking row `[0]`, which is not deterministic and could land on either of two periods legitimately covering one date (`scheduling_periods_no_overlap` excludes only _same-profile_ overlaps, so a compiled season and a surrounding `regular_school_year` row may both cover it) — when it landed on the empty published one, the roster came back empty and the AI panel refused with "No workers have submitted preferences for this period."

**Publish operation.** The schedule builder edits a single **template week** (the build week resolved above — the UI shows and drafts only that week). Drafts therefore
describe a **recurring weekly pattern**, not a one-off week. `publish_schedule` iterates blocks `BETWEEN v_period.start_date AND v_period.end_date`, so the builder resolving its week from the period is what keeps the grid and the operation it feeds in agreement. When the SM clicks Publish
...
**Step: hmod_notify_allied.** Opens a tracked **Allied coverage request** via `open_allied_coverage_request` (migration `20260729000010`) rather than emitting a single notification. The request row (`allied_coverage_requests`) carries the house, the coverage window, the reason, the current ladder rung, and the close-out outcome. Blocks stay `vacant` until the request is closed; see "Auto-staffing on close" below for what happens then. (Corrected 2026-08-07: this previously said the block flips to the `allied` status on confirmation. The `allied` enum value exists but nothing ever wrote it — closing as `allied_secured` now names the Allied contractor account on the seat directly, `status = 'claimed'`, per the amendment in `20260805000002`.)
...
- **Auto-staffing on close (added 2026-08-07, migration `20260807000002`).** `close_allied_coverage_request` now takes a sixth parameter, `p_assign_self boolean DEFAULT false`, and derives an assignee: the Allied contractor account for `allied_secured`; the acting `p_user_id` for `covered_internally` when `p_assign_self` is true; otherwise `NULL` (no write) for `covered_internally` without the flag, `desk_unstaffed`, and `no_longer_needed`. When an assignee is derived, the RPC computes every block in `[window_start_at, window_end_at)` at the request's house that is CURRENTLY `status = 'vacant'` and PERFORMs `admin_assign_worker(p_user_id, block_ids, assignee, 'this_week', true, p_now)` — delegating the actual write, not reimplementing it. This is deliberate: `admin_assign_worker` already carries the two invariants this feature must not weaken — the Harnwell training trigger (`enforce_harnwell_assignment_training`, fires regardless of caller) and the RSM own-house-only self-assignment guard (`v_is_rsm`, scoped to the RSM role's OWN house, distinct from `user_can_build_schedule`'s cross-house schedule-admin grant for hm/bm/rsm). The Allied contractor is exempt from both (`user_is_allied_contractor`, `20260805000002`) since it has no home desk and no training requirement. `p_override_advisories = true` unconditionally: the manager already committed by tapping the action, and this flow has no confirm-dialog step. If `admin_assign_worker` raises (most likely `cross_house_not_supported` for a self-cover attempt outside the acting manager's own house), the exception propagates and the WHOLE close aborts in the same transaction — the request stays open, not closed with an outcome the schedule does not reflect. The mobile Coverage sheet's "I can cover it" action is the only caller that ever sends `p_assign_self = true`; the web Action Inbox's generic outcome radio, and the mobile sheet's own "Covered internally" list row, both omit it (default `false`), since neither implies the closing manager personally covered the shift.
