...
### 1.6 Simultaneous Actions on the Same Seat

Two people acting on the same seat at the same instant is ordinary, not exceptional: two workers tap claim on the last open seat in the same second, a manager assigns someone while a worker is claiming, a worker drops a shift a moment after a swap for it was accepted. The following guarantees hold under those conditions, and are stated here because every one of them is a property of the system as a whole rather than of any single screen.

**A seat has exactly one owner.** Whoever completes their action first holds the seat. The person who loses the race is **told they lost** and their action has no effect; no action ever silently overwrites a seat that changed hands after it started. In practice this means a drop of a shift that is no longer yours is refused as not-yours; accepting a swap whose shift has since moved is refused as invalidated, and the swap is voided rather than left pending; and a manager whose assignment loses the race is told the seat was not assignable rather than being shown a success that erased somebody. A worker who is told their claim, drop, pickup, or swap succeeded genuinely holds what they were told they hold.

**A worker occupies at most one seat of a block.** No mechanism, including a manager override or a multi-week permanent pickup, can place the same worker on two seats of the same 30-minute block. Where an action would do so, that block is skipped and the rest of the action proceeds.

**A worker is never double-booked across houses.** Two claims fired at the same instant for two different desks at the same block start cannot both succeed; the second is refused as a time conflict. The same applies to the weekly hours cap: simultaneous claims cannot combine to exceed a hard cap.

**Coverage decisions are made at the moment they are acted on.** The system's judgement that a desk is empty is re-made at the instant it secures coverage, not carried over from an earlier reading. If a desk is staffed in the interval between the system noticing it was empty and acting on it, the securing step stands down: no floater is pulled from another house, no Allied coverage is procured, and the desk's remaining vacant seats are **not** locked. This matters because both a float and a locked desk are irreversible by automated means (Sections 5.5 and 7.4), so acting on a stale reading cannot be undone by the system afterward.

**Float guards are evaluated against the state at the moment of assignment.** A source desk is never emptied by two floats that each independently observed enough staff to spare one (Section 1.2), and two floats are never sent to the same desk because each was unaware of the other in flight. Where either would occur, the second float is refused and the gap re-escalates normally.
...
**The Archive tab's history holds every request, closed or not, for one month.** _(Amended 2026-08-05.)_ Unlike the live Coverage tab (which shows only open requests), the Archive tab's history table includes closed requests too, with their recorded outcome, so a manager can review what happened to a request after the fact — most commonly when approving hours. It defaults to showing the last 24 hours and can be widened to the last week or the last month; requests older than one month drop out of this view (the underlying record is retained, just no longer surfaced here). This history previously lived on a separate standalone page; it was folded into the inbox because both read the same underlying coverage-request data and a manager only ever needed one surface for it.
...
"Because it would be empty" is judged when the step fires, not earlier (Sections 1.6, 5.4). A block that was empty when the system last looked but has a worker on it by the time the step is due does **not** reach the step and is **not** locked. This is the only thing that keeps the one-way lock from being able to strand a staffed desk's remaining seats.
...
**The eligibility rules above are re-checked when the float is written, not only when the candidate is chosen** (Section 1.6). Two guarantees follow, and they hold no matter how many floats are being decided at once and whether each was started automatically or force-triggered by a manager. First, a source desk is never emptied: if the workers this float would take, combined with those another float has already taken, would leave a source desk with nobody on it, this float is refused outright rather than partially applied. Second, a destination desk never receives two floats for the same gap: if another floater is already inbound, this float is refused. In both cases the gap is left as it was and re-escalates through the normal chain. Refusing is the only safe outcome, because once a float exists it may not be withdrawn automatically (Section 7.4).
