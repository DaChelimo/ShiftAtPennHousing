 ...
 * `@upenn.edu` (the pre-migration domain) sees the same steer on web as on the app.
 * Never blocks sign-in — the account may legitimately use a different domain.
 *
...
export function domainWarning(email: string): string | null {
  ...
  if (lower.endsWith('@shiftatpenn.com') || lower.endsWith('@gmail.com')) return null;

  return 'Your email should most likely end with @shiftatpenn.com or @gmail.com, but mostly @shiftatpenn.com.';
}
