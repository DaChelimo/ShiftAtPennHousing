...
- `supabase/tests/phase-11-break-transitions.sql` — pgTAP: function existence,
  ...
  after T-1d, the winter closed-house / Harnwell gate, the cap-by-break-type
  (`effective_weekly_cap`), and the zero-hours opt-out storage + read. References
  functions/tables the phase-11 migration has not yet added → **TDD-red**.
  **48 assertions.**
...
-- Zero-hours opt-out storage + read (§4.4 "Indicating zero break hours" / ARCH §2.9).
break_optouts(break_id uuid FK break_periods ON DELETE CASCADE,
              user_id uuid FK users, opted_out_at timestamptz,
              PRIMARY KEY (break_id, user_id))
  -- the break analogue of the §4.1 "no hours" button; per (break, worker); RLS
  -- own-row write + service-role bypass (same migration that creates it).
worker_opted_out_of_break(p_user_id uuid, p_break_id uuid) RETURNS boolean
  -- EXISTS(SELECT 1 FROM break_optouts WHERE break_id=p_break_id AND user_id=p_user_id).
  -- The orchestrator reads it to fill `hasIndicatedZeroHours` for the pure
  -- selectBreakClaimNagRecipients; the opt-out is advisory (NEVER gates claiming).
```
...
| 14  | Zero-hours opt-out — per-break, advisory | The §4.4 "no break hours" indication is stored per `(break_id, user_id)` in `break_optouts` and read via `worker_opted_out_of_break` to fill `hasIndicatedZeroHours`. PER-BREAK: opting out of one break does not opt out of any other. ADVISORY: it only suppresses the T-3d nag — it NEVER gates claiming (a worker may opt out then claim, §4.1 latitude). The pure `selectBreakClaimNagRecipients` is unchanged; only the SQL storage + read are added. | BSpec §4.4 ("affirmatively indicated they want zero hours"; "Indicating zero break hours") / §4.1 (the regular-year "no hours" precedent); ARCH §2.9 (`break_optouts`). Closes the gap the first cut flagged — decided author-side, not by the implementer. |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| A. Function existence — `break_claim_phase`, `break_is_highlighted`, `open_break_claim_calendar`, `break_claim_calendar_pool`, `claim_break_shift`, `break_optouts` table, `worker_opted_out_of_break`    | 7          |
...
| J. Zero-hours opt-out — `break_optouts` row → `worker_opted_out_of_break` true; no row → false; per-break (Thanksgiving opt-out ≠ spring break)                                                           | 3          |
...
- **The "zero hours" opt-out _button_ and nag delivery — but NOT its storage.**
  The opt-out STORAGE is now in scope and pinned (decision #14): spec §4.4 +
  ARCH §2.9 define `break_optouts`, and the pgTAP §J exercises the table and the
  `worker_opted_out_of_break` read that fills `hasIndicatedZeroHours` for the pure
  `selectBreakClaimNagRecipients`. What remains out of scope is the **UI control**
  that writes/clears the opt-out row, the **T-3d nag job** that joins the opt-out
  read with the claimed-shift snapshot and delivers the alert, and the
  **candidate-worker enumeration** (which active workers of the break's operating
  houses are nag candidates) — all orchestrator/Edge-Function/UI territory, like
  the rest of the scheduling layer above.
...
- `phase-11-break-transitions.sql` calls `break_claim_phase` /
  `break_claim_calendar_pool` / `claim_break_shift` / `worker_opted_out_of_break`
  etc. and references the `break_optouts` table, none of which the phase-11
  migration has added yet → red (the `has_function` / `has_table` checks fail;
  the first call of a missing function aborts the run, exactly as phase-10's
  pgTAP does on its missing RPCs).
