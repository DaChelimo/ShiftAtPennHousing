...
| **Coverage & open-shifts**                           | NEW → **BUILT, read-only** (`/coverage`)        | Read ✓ wired in `lib/data/coverage.ts` (vacant / pending_float_in / allied blocks ≤30d + `block_step_status` escalation + floater from the pending_float_in row; permanent openings = vacant+`permanent_drop`, deduped by recurring slot). Gated SM/HM/BM. ⚠ **Force-trigger float** (a flow) + **"Call Allied / Mark covered"** (no RPC) are surfaced **disabled + flagged**; "View on calendar" is wired. |
| **Action inbox**                                     | NEW → **BUILT, read-only** (`/inbox`)           | Read ✓ wired in `lib/data/inbox.ts` over the signed-in user's `notifications` (own-select RLS → authed client). The Allied alert = `hmod_urgent` payload `{house_id, block_start_at, reason}` → house / window / reason. ⚠ actions (Call Allied / Mark covered = no RPC; mark-read = `mark_notification_read`, deferred) are **disabled + flagged**.                                                        |
...
| **Schedule builder**                                 | EXISTING → **RESKINNED** (`/schedule-builder`)  | ✓ (`getBuilderData`, drafts, `publish_schedule`). Visual reskin only — the cell-based drag mechanic + every testid/ARIA role preserved (e2e contract). The design's absolute drag-band + resize handles were NOT ported (they'd change the tested drag).                                                                                                                                                    |
