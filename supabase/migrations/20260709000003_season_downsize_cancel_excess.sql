-- Migration: Operating Seasons — downsize CANCELS excess workers (no grandfathering).
--
-- Supersedes the grandfather-on-decrease behavior of apply_compiled_season
-- (20260702000006). Stakeholder decision 2026-07-09: when a config change shrinks a
-- future block's headcount (double -> single) and MORE workers hold it than it now has
-- seats, grandfathering would leave two people booked on one seat with nobody told who
-- actually works it. Instead we CANCEL the excess and notify them, exactly like a
-- house-close does — just partial.
--
-- WHO is cut, in order:
--   1. external floaters first (floated_in / pending_float_in) — borrowed staff go
--      back before a home-rostered worker loses a shift;
--   2. then the SHORTER shift (fewest occupied blocks at that house on that NY date)
--      — least to lose;
--   3. then a deterministic tiebreak (assignment_id).
-- The rank-2 key is per-(worker,house,date), so the same worker is chosen across every
-- block of the overlap → a coherent cut, not a fragmented one. Beyond that we stop
-- optimizing: SM/RSM/HM can hand-tune the schedule from a sensible starting point.
--
-- The `blocks_grandfathered` impact counter is retired (a decrease now always resolves
-- to cancellations, counted in assignments_cancelled / floats_voided). Everything else
-- — config materialization, void-on-close, headcount INCREASE, dry-run rollback — is
-- unchanged from 20260702000006. season_target_headcount is unchanged and reused.
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Staffing patterns: replace this profile's rows (both day types).
    ...
    -- 4. Generate any missing blocks for newly-open houses/dates (idempotent).
    ...
    LOOP
      ...
      ELSE
        ...
        IF v_target > v_current THEN
          ...
          -- Add vacant seats so the live block reaches the new headcount.
        ...
        ELSIF v_target < v_current THEN
          ...
          IF v_occupied_now > v_target THEN
            ...
            -- Notify the cut workers (shift kind) + count.
            INSERT INTO notifications (recipient_user_id, type, scheduled_for, payload)
            SELECT a.user_id, 'personal_shift', v_now,
                   jsonb_build_object(
                     'kind', 'shift_cancelled_config',
                     ...
                     'block_start_at', v_blk.block_start_at)
            FROM shift_block_assignments a
            WHERE a.assignment_id = ANY (v_victim_ids)
              AND a.user_id IS NOT NULL;
            GET DIAGNOSTICS v_seat_gap = ROW_COUNT;
            c_assignments_cancelled := c_assignments_cancelled + v_seat_gap;
            ...
            -- Void any inbound float landing on a cut seat + notify + count.
            IF jsonb_array_length(c_affected) < c_affected_cap THEN
              c_affected := c_affected || COALESCE((
                SELECT jsonb_agg(jsonb_build_object(
                         'house', h.name, 'worker', u.name,
            ...
            -- Cancel the seats (history preserved); drop leftover vacant seats — the
            -- block is now full at its new target via the surviving occupants.
            UPDATE shift_block_assignments
            SET status = 'cancelled_config', vacancy_origin = 'none'
          ...
          ELSE
            -- Occupied already fits under the new target: just trim excess vacant seats
            -- down to (target - occupied). Nobody is removed.
...
-- rollback:
-- (restore the 20260702000006 body — grandfather-on-decrease + blocks_grandfathered.)
